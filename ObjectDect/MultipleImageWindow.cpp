...
{
	this->window_title=window_title;//this is a pointer to this class object?
	this->cols=cols;
	this->rows=rows;
...
void MultipleImageWindow::render()
{
	...
	{
		//circle the all image in the vector<Mat>
		string title=*titles_it;//There can we reconrize the iterator is a address of vector
		int cell_x=(cell_width)*((i)%cols);
		int cell_y=(cell_height)*floor((i)/(float)cols);//floor函数--向下取整
        ...
        putText(cell, title.c_str(), Point(20,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(200,0,0), 1, 1);//There is different from the last version
        i++;
        ++titles_it;
        if(i==max_images)
            break;
